...
1. In your `recipe.html` file, find your `<figure>` tag and add the id called "main-recipe-photo": `<figure id="main-recipe-photo">`. Save the HTML file.
...
We need to do the same here for ordered lists. We'll also add a little margin under each list item so they can breathe.
...
But we have one list, which is a vertical structure. We could separate these items into two sections and float them, but if they were generated from an application or CMS of some kind, we might not know when to separate the left section from the right.
