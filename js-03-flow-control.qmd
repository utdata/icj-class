...
How might we test if the middle name is an initial? And then what might we do if that test is true? If our middle name is only one character, then it would be an initial, right?
...
5. After your function closes (the last `}`), add these console.log tests:
...
6. Save your code and refresh your page so you can see the name. "John F Kennedy" needs the period after the initial, which we'll tackle with an `if` statement.
...
4. Add another console.log with another name that uses a middle initial and make sure that works, too.
