...
::: callout
- Add signals to save and push git
- Mid-update. Need to work HTML/CSS together
:::
...
1. At the top of the file, type `html` and you'll see some selections come up. Use the arrow key on your keyboard to choose the option `html:5` and hit return. You _should_ get all the boilerplate elements you need for an HTML page.
...
1. Control-click on [this link](resources/html-practice-text.txt) to open the text we need into a browser window, then highlight and copy/paste the contents.
...
1. **Copy** the text "Creamy Baked Macaroni and Cheese" and paste it inside a `<title>` tag in the head of the document. The `<title>` tag are the words that show on the tab of your browser.
1. Go back to the body of the text and add `<h1>` tags around that line to make a main headline.

When you did this, VS Code likely added both the opening and closing `h1` tags, and you had to cut 'n' paste or delete/retype the closing tag in the correct place. Let's introduce [Emmet](https://docs.emmet.io/), which is a shorthand syntax and tool for programmers to help write code blocks, and it is [baked into VS Code](https://code.visualstudio.com/docs/editor/emmet).

The next line "Brought to you by New York Times Cooking" we'll use as a `h2` headline, and we'll want to use an anchor tag around "New York Times Cooking" to link to the original recipe. We'll use Emmet to do it.

There are two parts to using Emmet to mark up existing code:

- You highlight the text
- You invoke the Command Pallette in VS code and choose the **Emmet: Wrap with Abbreviation** function
- Then you choose the tag or tags to apply based on Emmet shorthand. The shorthand isn't hard as it is based on the tags themselves without the angle brackets.
...
I'm not going to get into the difference between `id=` and `class=` here, as that is the next lesson, but I do want to show how Emmet allows us to add a class while marking up text. This time I'll give the steps, then show it.

1. Highlight the text "By Eric Kim".
2. Do **command-shift-p** to bring up the Command Pallette.
3. The Emmet: Wrap with Abbreviation command is probably at the top since you used it last, so you can just hit **Return**.
4. For the Emmet shorthand, use `p.byline`.

The `p` part is to add the `<p>` tag. The `.` says to add a "class" designation to the element, and `byline` is the name of the tag. If we wanted to add an `id=` instead, it would be `p#byline`.
...
I would now get in the habit of saving your work after each section we do here, and then check to see how it looks.

### Description lists

This next bit of text we need to put together as a [description list](https://www.w3schools.com/html/html_lists_other.asp), which is a special list construct where you have a "term" and a "description". Look at that W3schools link to see how it works.

The description list is a little different because the list "items" come in two parts, the `<dt>` for the "term" and the `<dd>` for the description for the term. We can use Emmet to get us real close with the text we have, but we'll need to do a little correction at the end.

With this Emmet command we'll introduce nested elements. By using the Emmet shorthand `dl>dt*` I'm saying "Wrap all of this with a `<dl>` tag, then inside of that (the `>`) wrap eleents with `<dt>` and then do it individually for all of them (the `*` part.)

Watch what happens to the text as I add each part of the command. Note I don't quite get exactly what I want since I can't alternate the `<dt>` and `<dl>` tags (or at least I don't know how), so I have to fix those manually.

![Emmet description list](images/emmet-wrap-dl.gif)

Now you need to do the same with your code.

1. Use the emmet shorthand `dl>dt*` to wrap create your description list.
...
One of your Codecademy lessons was about Semantic HTML, we we'll apply some of that here. From our lessons, we know that the `<img src="">` tag can display an image, but to make this image accessible and useful to all users of the web, we need to add an `alt=` attribute. It's also best to wrap both the image and caption in a semantic `<figure>` tag.


So far, we've used Emmet to wrap existing code, but we can also write shell code using emmet, and then type in the elements, tabbing through different parts to fill them in.

Again, this might be helpful to see before you do it on your own. This is me adding the figure, img and figcaption tags we just talked about, and then filling the values.
...
1. On a blank line above the caption in your text, add the following emmet shorthand, then use tab to complete it.
...
1. Code the intro with the emmet shorthand `p.intro`.
...
2. Use the emmet Wrap with Abbreviation with the shorthand `ul>li*` to code the ingredients list.
3. Skip to the "Nutrition" section and do the same there.
...
Now, if you check your live server again your page should look like the examples we started with at the beginning of this lesson, but we aren't quite done yet.
...
2. Use a `<header>` tag two wrap from before the main headline to past the definition list. In other words, stop it before the `<figure>` tag, which we've already added.
3. Add separate `<section>` tags around the Ingredients, Preparation and Nutrition sections.
4. Add a new `<footer>` and child `<p>` tags **after** the closing `</article>` tag. Inside the paragraph tag, add the text "A class project by First Last" but with yoru name.
