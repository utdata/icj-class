...
::: callout-tip
If you have a problem with `gulp fetch` and get an error, you can use [these steps](../appendices/a-fetch-problems.qmd) to download the data. Do that and then move on with the rest of the lesson, but reach out to me to see if we can fix the problem.
:::
